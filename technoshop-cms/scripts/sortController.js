import { wrapperSort } from "./elements.js"
import { getGoods } from "./serviceAPI.js"
import { tableRender } from "./tableView.js"

export const sortController = () => {
    wrapperSort.addEventListener('click', async ({target})=>{
        const sortField = target.closest('[data-sort]').dataset.sort
    
        if(sortField){
            const goods = await getGoods()
            
            switch (sortField){
                case 'id':
                    if(target.dataset.direction === 'up'){
                        target.dataset.direction = 'down'
                        goods.sort((a,b)=>a[sortField] > b[sortField] ? 1 : -1)
                        
                    }else{
                        target.dataset.direction = 'up'
                        goods.sort((a,b)=>a[sortField] > b[sortField] ? -1 : 1)
                    }
                    tableRender(goods)
                    break
                case 'title':
                    if(target.dataset.direction === 'up'){
                        target.dataset.direction = 'down'
                        goods.sort((a,b)=> b[sortField].localeCompare(a[sortField], 'ru'))
                        
                    }else{
                        target.dataset.direction = 'up'
                        goods.sort((a,b)=> a[sortField].localeCompare(b[sortField], 'ru'))
                    }
                    tableRender(goods)
                    break
                case 'category':
                    if(target.dataset.direction === 'up'){
                        target.dataset.direction = 'down'
                        goods.sort((a,b)=> b[sortField].localeCompare(a[sortField], 'ru'))
                        
                    }else{
                        target.dataset.direction = 'up'
                        goods.sort((a,b)=> a[sortField].localeCompare(b[sortField], 'ru'))
                    }
                    tableRender(goods)
                    break
                case 'price':
                    if(target.dataset.direction === 'up'){
                        target.dataset.direction = 'down'
                        goods.sort((a,b)=>a[sortField] > b[sortField] ? -1 : 1)
                        
                    }else{
                        target.dataset.direction = 'up'
                        goods.sort((a,b)=>a[sortField] > b[sortField] ? 1 : -1)
                    }
                    tableRender(goods)
                    break
            }
        }

    })
}